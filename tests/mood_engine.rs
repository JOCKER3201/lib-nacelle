...
fn the_masters_alarm_skin_is_reachable_and_changes_what_would_be_drawn() {
    ...
    // `glow.panel_edge.enabled` is false in the resting master and true in
    // `[mood.alert]` — image 4's screen-wide lit edges, and one flag that
    // says whether an overlay reached the bake at all.
    let glow = theme::id("glow.panel_edge.enabled").expect("the master declares the panel glow");
    assert!(!theme::resolved().flag(glow), "the resting theme already glows");
    ...
    // Lockdown is alert plus a data hue: it inherits, so it glows too.
